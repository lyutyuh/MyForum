...
public class Forum extends Model {
    ...
    public List<User> getAllRepliedUsers(){
        return User.find("select u from User u, Post p where p.topic.forum=?1 and p.topic.postedBy=u group by u.id order by count(distinct p.id) desc", this).fetch();
    }
    
    public List<User> getAllPostedUsers(){
        return User.find("select u from User u, Post p where p.topic.forum=?1 and p.postedBy=u group by u.id order by count(distinct p.id) desc", this).fetch();
    }

    // TODO CHECK
    public List<User> getActiveUsersByPost() {
        return User.find("select u from User u, Post p where p.topic.forum=?1 and p.postedBy=u group by u.id order by count(distinct p.id) desc", this).fetch(10);
    }
    
    

    public List<Long> getUsersPostnum() {
        return User.find("select count(distinct p.id) from User u, Post p where p.topic.forum=?1 and p.postedBy=u group by u.id order by count(distinct p.id) desc", this).fetch();
    }
    
    public List<Long> getUsersReplnum() {
        return User.find("select count(distinct p.id) from User u, Post p where p.topic.forum=?1 and p.topic.postedBy=u group by u.id order by count(distinct p.id) desc", this).fetch();
    }
